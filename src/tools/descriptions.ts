...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  'sftp-upload': 'Upload a file to the remote server via SFTP (secure file transfer, not shell-based). Replaces an existing file at that path unconditionally — there is no overwrite flag; use sftp-upload-file if you need the destination to be new. The approval prompt and the audit record carry the byte count and a short digest of the content, so two uploads to the same path are distinguishable.',
